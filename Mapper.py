...
for line in sys.stdin:
    ...
    for i in author.split('::'):
        ...
        for j in words:
            #translator = string.maketrans(None,string.punctuation)
            #j=j.translate(translator)
            j=j.translate(None, string.punctuation)
            if len(j) > 1 and j not in stopwords.keys():                        
